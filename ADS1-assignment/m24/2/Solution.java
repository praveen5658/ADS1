...
class Student {
     ...
     * @return     { description_of_the_return_value }
     */
    public boolean compare(final Student other) {
...
class Bst {
    ...
    public Node put(final Node x, final Student key) {
        ...
        boolean cmp = key.compare(x.getKey());
        if (cmp) {
    ...
    //     return null;
    // }
    public void checkless(final double val){
        Node x = root;
        while(x!=null){
            if (x.getKey().getmarks() <= val){
    ...
    public void checkgreat(final double val){
        Node x = root;
        while(x!=null){
            if (x.getKey().getmarks() >= val){
                System.out.println(x.getKey().getname());
...
public final class Solution {
    ...
    public static void main(final String[] args) {
        ...
        for (int i = 0; i < queries; i++) {
            ...
            switch(input[0]){
                case "LE":
                    bst.checkless(Double.parseDouble(input[1]));
                    break;
                case "GE":
                    bst.checkgreat(Double.parseDouble(input[1]));
                    break;
            }
        }
    }
